  ...
  const sortedAndGroupedData = useMemo(() => {
    ...
    if (groupByField) {
      ...
      result.forEach((item, index) => {
        ...
        // Handle date normalization if value is a date
        if (groupValue instanceof Date || !isNaN(Date.parse(groupValue))) {
          // const date = new Date(groupValue);
          // groupValue = date.toISOString().split("T")[0];
          // Convert to YYYY-MM-DD (removing time)
          const date = new Date(groupValue);
          const yyyy = date.getFullYear();
          const mm = String(date.getMonth() + 1).padStart(2, '0'); // getMonth is 0-based
          const dd = String(date.getDate()).padStart(2, '0');
          groupValue = `${yyyy}-${mm}-${dd}`;
    ...
    <div className="w-full space-y-4">
          ...
          <div className="flex items-center flex-wrap gap-2 pl-8 pr-3 py-2 bg-background border rounded-md">
            ...
            {searchQuery && (
              <Badge variant="secondary" className="flex items-center gap-1">
                ...
                {autoDetectedColumns[searchColumn]?.label} : {searchQuery}
      ...
      <div className="flex flex-wrap gap-2">
                ...
                <div className="space-y-3">
                  ...
                  {filterState.rules.length === 0 && (
                    <div className="text-center text-muted-foreground py-4">
                      No filter rules. Click on "Rule" to add one.
                    </div>
                  )}
                </div>
              </div>
        ...
        <Select onValueChange={(value) => setSearchColumn(+value)}>
          <SelectTrigger className="w-[180px]">
            <SelectValue placeholder="Column to search" />
          </SelectTrigger>
          <SelectContent>
            <SelectGroup>
              <SelectLabel>Column</SelectLabel>
              {autoDetectedColumns.map((column, index) => (
                  <SelectItem key={column.key} value={index + ""}>{column.label}</SelectItem>
              ))}
            </SelectGroup>
          </SelectContent>
        </Select>
